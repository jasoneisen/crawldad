...
/// <summary>The portal's boot-time key-ring guard (<see cref="DataProtectionOptionsValidator"/>): neither knob set (the
/// dev/test default) passes, both set as absolute URIs passes, and every half-configured or malformed shape fails
/// startup with a specific message — so a partially-configured portal host can never silently fall back to the ephemeral
/// ring (which would rotate auth cookies AND orphan the stored tenant keys on every redeploy).</summary>
public class DataProtectionOptionsValidatorTests
{
    ...
    {
        var result = Validate(new DataProtectionOptions { KeyRingBlobUri = "dataprotection-portal/keyring.xml", KeyVaultKeyId = _key });
