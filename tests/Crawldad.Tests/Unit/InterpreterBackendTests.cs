...
public class InterpreterBackendTests
{
    ...
    [Fact]
    public async Task A_connect_failure_is_not_retried_by_the_program_retry_policy()
    {
        // config.retry wraps only the POST-connect program; the connect boundary has its own separate knob
        // (config.connectRetry), so config.retry — however generous, even listing backend_unavailable in retryOn — must
        // NOT re-attempt the connect. With no connectRetry here, the run is a terminal backend_unavailable on the first
        // failure, with the connect tried exactly once (the pre-existing single-shot default is unchanged).
        const string Payload =
            """
