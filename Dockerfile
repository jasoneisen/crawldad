# Crawldad.Api — multi-stage container image.
#
...
# runs `dotnet Crawldad.Api.dll db-apply`), never on a normal server start — the same argument-driven entrypoint
...
COPY src/Crawldad.Api/Crawldad.Api.csproj src/Crawldad.Api/
RUN dotnet restore src/Crawldad.Api/Crawldad.Api.csproj

# Source layer: the web/contracts source, plus the two repo-root files embedded into the assembly as resources
# (schema/crawldad-1.schema.json and llms.txt — referenced by Crawldad.Api.csproj via ..\..\). The Fixtures under
# src/Crawldad.Api ship as content (the `fake` browser backend loads them at runtime), so they publish with the app.
COPY schema/ schema/
COPY llms.txt ./
...
# so any warning fails this build exactly as CI does. UseAppHost=false: we launch via `dotnet Crawldad.Api.dll`,
...
RUN dotnet publish src/Crawldad.Api/Crawldad.Api.csproj \
...
ENTRYPOINT ["dotnet", "Crawldad.Api.dll"]
