...
| **`RunExecutorSaga`** (`mt_doc_runexecutorsaga`) | the run's `script` + `inputs` — the resume source (re-run + re-connect after a restart) | no (inputs are caller config; credentials among them are refs) | **reclaimed at terminal (CD-5)** — the shared finaliser's `RunFinished` `MarkCompleted()`s the saga the instant the run finishes (deleting the document), and the already-scheduled `RunDeadline` doubles as the crash-safe janitor if that signal is lost between the terminal commit and cleanup — so retention is **bounded to the run's own duration** (worst case its `deadlineMs`), not "until the schema is dropped" |
...
now also asserts the saga is **reclaimed once the run finishes** (CD-5), so even the by-reference inputs no longer linger
indefinitely at rest.
