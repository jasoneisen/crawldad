...
| **Marten events** | `Features/Runs/RunEventScrubber`, at both append paths: the `POST /runs` append (`StartRunEndpoint`, sync) and the executor's own-session append (`RunExecutor.ExecutorObserver`, async, WP2) | Every event is scrubbed before append: `RunStarted` (payload name + input key names), `LogEmitted`, `RunFailed`, and the WP3 step trace (`Navigated`/`Clicked`/`Filled`/`Extracted`/`Downloaded`/`Screenshotted`/`StepFailed`/`RunSessionOpened`) — a CD-6 `Filled` carries `secret:<refName>` (the reference name), never the typed secret. Input *values* are never persisted; `Extracted` carries a shape ref, `Downloaded` a blob ref, `Screenshotted` (the explicit `screenshot` node, #8) and `StepFailed` a screenshot ref — metadata only (§12). |
...
| **`RunTimeline` + `GET /runs/{id}/timeline`** | — (by construction) | folds the already-scrubbed step trace; surfaces extracted-value refs, blob refs, region, the explicit-screenshot refs (#8), and the failure screenshot ref — never a raw value or the image. |
| **Screenshot blobs** | `Infrastructure/Storage/IScreenshotStore` (content-addressed ref) | both the failure screenshot and the explicit `screenshot` node (#8) flow through this one seam: the image lives in a **deletable**, tenant-partitioned, TTL-governed blob store; the immutable `StepFailed`/`Screenshotted` event holds only the `screenshots/{sha256}.png` ref (§12 PII). **A screenshot — failure or explicit — captures the page as *pixels*, which cannot be scrubbed like text; a shot taken right after a `fill.secret` can therefore show the typed value.** The same mitigations apply to both: the ref-only immutable trace, the shorter 7-day screenshot TTL, per-tenant partitioning, and on-demand hard-delete erasure (`IRetentionStore.DeleteAsync`). Authors must not place a `screenshot` where a secret is rendered on-screen; the platform never routes the bytes anywhere but the deletable store. |
