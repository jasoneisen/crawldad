...
/// <summary>The configured tenant set: Crawldad is hosted-only and multi-tenant, and the tenant is the billing
/// subject, so tenancy identity is load-bearing. A config-bound list of tenants (id, api key, actor identity) under
/// <c>Crawldad:Tenants</c>; there is deliberately no tenant-management endpoint here.</summary>
...
public sealed class TenantDescriptor
{
    ...
    /// <see cref="Crawldad.Web.Features.Runs.RunLimitsOptions.MaxConcurrentRunsPerTenant"/>.</summary>
    public int? MaxConcurrentRuns { get; init; }

    /// <summary>An optional per-tenant override of the global admission-queue depth — the per-tier at-cap wait room
    /// before a further at-cap run is rejected <c>429 queue_depth_exceeded</c>. Null defers to the global
    /// <see cref="Crawldad.Web.Features.Runs.RunLimitsOptions.MaxQueueDepthPerTenant"/>.</summary>
...
public sealed class TenantRegistry
{
    ...
    /// too-short or duplicate key) throws at construction so the host fails loudly at boot.</summary>
    ...
    public TenantRegistry(IOptions<TenantOptions> options)
    {
        ...
        foreach (var tenant in options.Value.Tenants)
        {
            ...
            // A tenant id namespaces its per-tenant secret-vault keys (Secrets:{tenant}:{ref}); a ':' in the id would
    ...
    /// interrupted runs must be found and resumed under that tenant).</summary>
    public IReadOnlyCollection<string> TenantIds => [.. _entries.Select(e => e.Tenant.Id)];

    /// <summary>Resolves a presented API key to its tenant, or fails. Fixed-time over the whole set: every entry is
    /// probed (no early return) so neither the match position nor a near-miss is observable through timing.</summary>
    ...
    /// <summary>The tenant's configured admission-queue-depth override, or defers to the global default when absent.
    /// Looked up by the run queue; an unknown tenant id also defers to the global — same override shape as
    /// <see cref="TryGetConcurrencyOverride"/>.</summary>
