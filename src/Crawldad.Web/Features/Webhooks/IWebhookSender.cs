...
/// <summary>The real <see cref="IWebhookSender"/>: a plain <see cref="IHttpClientFactory"/> client (the same seam the
/// browserbase adapter uses), POSTing the signed body. A non-2xx response or any transport fault (connection refused,
/// DNS failure, per-attempt timeout) is reported as a non-delivery so the handler can retry; a real cancellation of the
/// caller's token is left to propagate.</summary>
