...
/// network/transport/5xx/throttle blip a bounded <c>config.connectRetry</c> should re-attempt) or
/// <b>auth-shaped/permanent</b> (a rejected key, a client-error 4xx other than the throttle codes 429/408, or an absent
/// credential — fail fast). The fault is only inspected here, never propagated, so a message that embeds a connect
/// URL/token still never leaves the boundary. The default for an unrecognised fault is non-retryable: a retry only ever
/// earns its keep against a known-transient shape, never a bug.</summary>
internal static class ConnectFaultClassifier
{
    ...
    public static bool IsTransient(Exception fault) => fault switch
    {
        ...
        // The hosted session API answered, or the socket faulted before it could: a null status means the request never
        // got an HTTP response at all (connection refused/reset, DNS, TLS) — a transient network fault; a 5xx is a
        // transient server fault; 429 (rate limit) and 408 (request timeout) are canonically transient throttling — a
        // retry with backoff is exactly the right response, and the WS-handshake path already retries them (it fails
        // fast only on 401/403); every other 4xx is a client/auth error (a rejected key, a bad request) a retry cannot fix.
    ...
    // A session-API HttpRequestException status: a null status (no HTTP response — a raw socket/DNS/TLS fault) or a 5xx
    // is transient, as are 429 (Too Many Requests — rate limiting) and 408 (Request Timeout): canonically transient
    // throttling a bounded retry should ride out, not auth-shaped. Every other 4xx is the auth/client class that fails
    // fast. Mirrors the WS-handshake split below, which likewise fails fast only on the auth-shaped 401/403 — so the
    // same throttle retries whether it arrives over the HTTP session API or the WS handshake (issue #85).
