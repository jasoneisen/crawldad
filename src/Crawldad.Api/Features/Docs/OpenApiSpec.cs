...
public static class OpenApiSpec
{
    ...
    [
        ...
        new("delete", "/runs/{id}", "eraseRun", "Erase a finished run's result, read models, and event stream.", _runs, Anonymous: false, [Id], null,
            ...
            Description: "On-demand right-to-erasure for a FINISHED run (issue #71): hard-deletes the run's stored result (RunProgress), all three of its derived read models — the Run snapshot, the RunTimeline, and the RunSummary row GET /runs lists — and its event stream: the bulk result body, the headline metadata a list row would otherwise still show, and the incidental PII a scrubbed timeline can hold (a log message, a navigated URL). Tenant-scoped: an unknown, foreign, already-erased, or purely-synchronous run (which stores no progress row) is a 404 with no existence oracle, so a repeated DELETE is idempotent (204 then 404). A still-active run (running/queued) is a 409 run_still_active — cancel it first."),
