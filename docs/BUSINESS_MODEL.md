...
- **Packaging intent (not yet mechanized):** seats, managed-payload counts, runs/month, per-tier retention windows, webhooks, and drift monitoring are product commitments the tiers describe; the underlying capabilities range from shipped (drift comparison via `GET /runs/{id}/drift`) to roadmap (scheduled drift monitoring → **issue #47**; webhooks → **issue #12**; timeline archival to customer storage for the Scale/Enterprise retention promise → **issue #46**).
...
## Open question — the Enterprise deployment boundary

Closed-source + hosted-only leaves no answer for the buyer who says "this must run inside our perimeter" (large farms, regulated industries, government). The candidate answer is an **operator-managed dedicated instance** (single-tenant Crawldad we deploy and operate in the customer's cloud or a VNet-peered enclave); the alternative is declaring those deals out of scope and staying purely multi-tenant. This decision sizes the addressable Enterprise segment and the ops burden, and should be made before the first farm-scale negotiation — everything else in the tier design survives either answer. Tracked in **issue #49**.
