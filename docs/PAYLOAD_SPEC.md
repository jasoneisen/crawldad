...
The `capture` node binds `{ contentId, sha256, sizeBytes, storedAs, stored }` (identical to `download`) so `result` returns a compact **manifest of refs**, never megabytes of HTML. The captured document is the customer's own content going to the customer's own storage, so — unlike a result-borne `innerHtml` — it never passes through the credential-param scrubber (a `token=…` in the page survives verbatim) and never lands in Crawldad's Postgres. `config.captureOnFailure: { to: <Expr→storageTarget> }` is the failure analog of `screenshotOnFailure`: on a step failure it banks the failing page's HTML to a tenant BYO target (absent ⇒ disabled — there is no default target).
