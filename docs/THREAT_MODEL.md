...
Payloads and events never carry a raw secret. A backend binding carries a **`credentialRef`** — an id into an `ISecretStore` (`Infrastructure/Security/ISecretStore.cs`). The secret is resolved **only at connect time**, by the adapter, and lives solely in the interpreter's memory for that session. `apiKey` mode stores a vault reference; `connectUrl` mode treats the whole URL as a one-time secret (still scrubbed). The default store is configuration-backed (`Secrets:{ref}`), so any provider (user-secrets, env vars, a mounted vault file) supplies secrets without a code change.

The vault is a **keyed-adapter registry** (`ISecretStoreRegistry`, the same pattern as browser backends and storage sinks): `config` ships today; a real `azure-keyvault`/`aws-secretsmanager`/`hashicorp-vault`/customer-HTTP adapter is one registration line with no change to the interpreter. Two resolution surfaces sit on the one adapter: the backend-connect path (process-global — an operator-configured account credential, `Secrets:{ref}`) and the form-fill path (tenant-scoped — `Secrets:{tenant}:{ref}`, where the tenant is the authenticated principal's, so a tenant can only ever resolve its own references).
...
| **HTTP response** | `StartRunEndpoint` (sync) / `GetRunEndpoint` (async poll) | the failure message and the shaped `result`/`partial` (`ScrubJson`) — caller data a scraped page could echo. |
...
- **Erasure.** `IRetentionStore.DeleteAsync` is the deletion primitive backing both the janitor and the on-demand **hard-delete** erasure path (a right-to-erasure request deletes the tenant's matching blobs; the immutable trace retains only the credential-free ref). Optional crypto-shredding (encrypt each blob under a per-run/subject key; discard the key to render it unrecoverable) is **designed, not built** — hard delete is the shipped erasure mechanism and suffices wherever storage is mutable.
