...
Cancellation is cooperative and honored **between steps**: the browser session is torn down cleanly (you cannot yank mid-step without leaking a backend session), and the run reaches `cancelled` with a `partial`. A queued run cancels straight to `cancelled` without ever consuming a slot. Promotion, cancel and the queue-wait timeout all serialize on the run stream's exclusive lock so exactly one of them drives a queued run terminal; a cancel that *loses* that race leaves the now-running run alone (still `202`), while one that cannot **take** the lock at all — a concurrent transition held it past the database command timeout — commits nothing and answers `409 run_claim_contended` rather than a `500`, so the caller can simply retry ([`API.md` §7](API.md#7-cancel--post-runsidcancel)).
...
- **Read models.** `RunStarted` **pins the exact payload revision + script hash**, so editing a payload never mutates historical runs and drift is detectable. The `RunTimeline` async projection is the lag-tolerant cross-run view (ordered steps, durations, refs, region) — daemon-built in production, so `GET /runs/{id}/timeline` can `404` for a run that exists but is not yet projected; `GET /runs/{id}` is the read-your-writes existence check that tells that apart from an unknown run ([`API.md` §9](API.md#9-drift-timeline-screenshots--erasure)). The lighter `RunSummary` projection is the one-row-per-run listing view behind `GET /runs`. On-demand erasure (`DELETE /runs/{id}`) deletes the run's stored result, its snapshot and both views, and its event stream in one transaction, but under the **async** lifecycle a daemon batch that had already read those events can still re-materialize a view after the erase commits — a known gap, so an erasure that must be provably complete should be re-checked (or the daemon paused) rather than assumed atomic.
...
- **Replay & drift.** Replay re-executes a historical run's **pinned revision** (the script is guaranteed identical; inputs are resupplied because input *values* are never persisted). Drift is detected **on read** and has shipped (issue #47): `GET /runs/{id}/drift` compares a run's pinned revision/hash against the payload head, and `GET /payloads/{id}/drift-status` folds a payload's per-selector miss history (the `SelectorMiss` trace events) into a baseline/delta assessment. What remains roadmap is turning that on-read signal into **scheduled monitoring/alerting** — the nightly canary that would re-run pinned revisions to supply the cross-run signal automatically is tracked in **issue #7**.
...
Designed but not built: a **`@puppeteer/replay` importer** (a one-way lift of a linear Chrome DevTools recording into a payload) — **issue #13**; it is not required by the current workload, which uses inputs, storage targets, and SSE instead. (**Webhooks** — issue #12, once sketched here as an `ISideEffect` sender for run-lifecycle notifications — have since shipped: run-lifecycle delivery with HMAC-SHA256 signing, exponential-backoff retry, and SSRF-guarded endpoints; see [`API.md` §14](API.md).)
