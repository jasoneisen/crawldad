...
- **Trace events** appended on the executor path: `RunStarted`, `StepStarted`, `Navigated`, `Clicked`, `Filled`, `Waited`, `Extracted`, `Downloaded`, `Screenshotted`, `StepFailed`, `LogEmitted`, `RunAttemptFailed`, `RunSessionOpened`, the checkpoint/resume/cancel markers (`RunCheckpointReached`, `RunResumed`, `RunCancellationRequested`, `RunCancelled`), the queue markers (`RunQueued`, `RunDequeued`), and the terminal `RunSucceeded`/`RunFailed`.
