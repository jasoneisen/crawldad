...
- **Solo dev → their own machine:** the documented on-ramp is a dev tunnel they already know (`ngrok`/`cloudflared` exposing local Chrome's `--remote-debugging-port`); the resulting public wss URL is the backend binding. The tunnel URL is **credential-bearing** — it is passed as a `credentialRef` and scrubbed exactly like a Browserbase connect URL (see [`THREAT_MODEL.md`](THREAT_MODEL.md)). Latency makes a public tunnel an authoring/debugging topology, not production — the natural upsell to a real backend. This on-ramp is documented step-by-step in [`TUNNEL_BACKEND.md`](TUNNEL_BACKEND.md), which also records the verified connect behavior: a failed connect is a terminal `backend_unavailable`, single-shot by default — but a bounded, opt-in `config.connectRetry { maxAttempts, delayMs }` re-attempts a **transient** connect fault (a tunnel reconnect, a refused/reset socket, a WebSocket handshake failure, a 5xx or a 429/408 throttle from a hosted session API), re-reading the `credentialRef` each attempt so a connector's mid-window re-registration (a rotated tunnel URL under a stable name) is picked up, while an **auth-shaped** fault (a rejected key, a client-error 4xx other than 429/408, an absent credential) still fails fast; `config.retry` governs only the post-connect program on the same session and never reaches the connect (issue #76). A hosted-side relay for reaching an on-prem browser with no public endpoint stays deferred to the Enterprise case.
...
| Container app (API) | `ca-crawldad-stg` | HTTP ingress 8080, **scale-to-zero** (min 0 / max 1), 0.5 vCPU / 1 GiB |
| Container app (portal) | `ca-crawldad-stg-portal` | Blazor SSR; HTTP ingress 8080, **scale-to-zero** (min 0 / max 1), 0.5 vCPU / 1 GiB; same identity/registry/env as the API |
...
Secrets are never committed: the Postgres password and a placeholder-tenant API key are generated at bootstrap and passed as GitHub environment secrets; the connection-string secrets are composed inside the deploy and stored in Key Vault, resolved passwordless by the app identity. The app runs with the **Azure Blob** storage provider and one placeholder tenant so the API is callable beyond the anonymous `/health`. Credentials registered via `/browsers` are encrypted at rest with an ASP.NET Data Protection key ring that is **persisted** — stored in the `dataprotection` blob container and wrapped by the Key Vault `dataprotection` key, reached passwordless via the app identity — so a registration survives a redeploy (critical under scale-to-zero, which restarts the container on effectively every deploy); see issue #65. Steady-state cost is **~$22–25/mo** (ACR Basic ~$5, Postgres B1ms + 32 GiB ~$16, storage/Key Vault/Log Analytics ~$2, Container Apps largely inside the free grant under scale-to-zero).
...
- **No out-of-band schema step; the schema is applied at startup instead.** Unlike the API, the portal has no `db-apply` command (its entry point only ever serves), so it calls `ApplyAllDatabaseChangesOnStartup()` **unconditionally, in every environment** — the `portal` schema is diffed and applied while the host boots, over the admin login that already backs the shared connection string (the DB principal therefore needs DDL rights on that schema). This replaces the former lazy first-document-use auto-create, whose first-touch DDL raced across the 1–3 production replicas. Marten's startup applier takes a Postgres advisory lock (`MartenActivator` implements `IGlobalLock` and hands itself to Weasel's locked apply overload), so concurrent replicas serialise; one that cannot attain the lock fails its own boot and Container Apps restarts it into the already-provisioned schema. A dedicated portal schema-apply job can still follow if the boot-time apply ever becomes too slow to sit in the startup path.
...
3. Provide the token to the deploy as the **`PORTAL_POSTMARK_SERVER_TOKEN`** GitHub Actions secret (staging; `PROD_PORTAL_POSTMARK_SERVER_TOKEN` for the future prod workflow), which the deploy workflow exports in its job `env:` so `readEnvironmentVariable` can read it (wired for staging in `.github/workflows/deploy-staging.yml`; the prod line lands in the prod workflow when prod is bootstrapped). Then run the deploy: the bicep creates the `portal-postmark-server-token` Key Vault secret and wires the portal env. *(This first tokened deploy is what creates the secret and the container-app email env; only afterwards can you rotate the token by editing the `portal-postmark-server-token` value in the vault + restarting the portal revision. Before that deploy the container references no email secret/env, so a direct vault edit has no effect. A later redeploy re-asserts the pipeline value, so the GitHub secret stays the canonical source.)*
